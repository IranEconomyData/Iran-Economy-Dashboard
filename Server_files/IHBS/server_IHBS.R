...
server_IHBS <- function(input, output){ 
  ...
  # 
  # #----
  # output$IHBS_HEduProv  <- renderPlotly({
  #   # Get the filtered data
  #   data <- iran_map |> select(geometry, Province) %>%
  #     left_join(IHBS_HEduProv_data(df_IHBS_HEduProv, input$IHBS_HEduProv_Year, input$IHBS_HEduProv_Region, input$IHBS_HEduProv_Category) |> select (Province, Value),
  #               by = "Province")
  #   centroids <- st_centroid(iran_map)
  #   
  #   p <- ggplot() +
  ...
  #     
  #   
  #     p <- ggplot(data, aes(x = ProvinceName , y=Value ,fill=ProvinceName)) +
  #       geom_bar(stat = "identity", position = "dodge")+ 
  #       coord_flip()+
  #       # scale_x_continuous(breaks = seq(87, 96, by = 1))  +
  #       # 
